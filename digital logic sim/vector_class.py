...
class Vector:
    ...
    def __sub__(self, other):
        # The second will sub his coordinates to the first
        x = self.x - other.get_x()
        y = self.y - other.get_y()
        name_a = self.name
        name_b = other.get_name()
    ...
    def __mul__(self, other: float):
        # Two vector can be multiplied together
        x = self.x * other
        y = self.y * other
    ...
    def get_normalised(self):
        ...
        assert self.x != 0 or self.y != 0, "Le vecteur nul ne peut pas être normalisé !"
